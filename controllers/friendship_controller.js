...
module.exports.newrequest = async function(req,res){
    try{
        let already = await Friendship.findOne({ $or: [ {from_user:req.user._id, to_user: req.params.id}, {from_user:req.params.id, to_user: req.user._id} ]});
        if(already){
        ...
        let fornew = await User.findById(req.params.id);
...
module.exports.delfriend = async function(req,res){
    try{
        let link = await Friendship.findOne({ $or: [ {from_user:req.user._id, to_user: req.params.id}, {from_user:req.params.id, to_user: req.user._id} ]});
        if(!link){
            return res.status(400).json({
                message: "Not Friends"
            });
